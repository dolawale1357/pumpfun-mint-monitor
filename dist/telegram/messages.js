...
export function buildTokenMessage(event) {
    const lines = ["🚨 <b>NEW PUMP.FUN TOKEN</b>", ""];
    ...
    if (event.initialBuySol !== undefined) {
        lines.push("", `<b>Developer Initial Buy:</b> ${formatSol(event.initialBuySol)} SOL`);
    }
    ...
    if (event.blockchainCreatedAt !== undefined) {
        lines.push(`<b>Blockchain Created:</b> ${formatIsoTimestamp(event.blockchainCreatedAt)}`);
    }
    lines.push("", "<b>Links:</b>", `Pump.fun: https://pump.fun/coin/${escapeHtml(event.mint)}`, `Solscan: https://solscan.io/token/${escapeHtml(event.mint)}`);
    return lines.join("\n");
}
